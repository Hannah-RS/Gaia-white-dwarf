"""
01/11/21
Function for importing Alex's 1.5Msol data
...
"""
def import_data_1p5(engulfed,jupiter_mass,log_mass,path=''):
    ...
    #import data for 1.5Msol star
    
    #import simulation A
    datac=pd.read_csv(sys.path[1]+"simulation_data/results_1p5_coarse.csv")
    datac.columns=['M','a_i','a_f','status']
    #import fine data - simulation B
    dataf=pd.read_csv(sys.path[1]+"simulation_data/results_1p5_fine.csv")
    dataf.columns=['M','a_i','a_f','status']
    

    #concatenate data frames
    data=pd.concat([datac,dataf])
